...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function QuotaPopover(props) {
			...
			return h("div", { style: { ...S.panel, ...anchor } },
				...
				h("p", { style: S.at },
					...
					(payload.model ? " · " + t("modelTag") + " " + payload.model : "")
				),
		...
		function FloatingWidget(props) {
			...
			useEffect(() => {
				...
				const read = () => {
					try {
						...
						setProvider(cur && cur.provider ? cur.provider : null);
						setModel(cur && cur.model ? cur.model : null);
					} catch {
						setProvider(null);
						setModel(null);
					}
			...
			// 拉取额度（host 按 provider 返回 visible + usage，按 model 返回对应限额）
			// 序号守卫：快速切换 provider/模型时，丢弃过期响应（旧账号数据不能覆盖新账号）
